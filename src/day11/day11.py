...
energy over time and flashes brightly for a moment when its energy is full.
...
# I have a suspicion that part 2 will be some crazy computation stuff on the array
# Turns out it wasn't that bad but cba to drop numpy now
import numpy as np
...
class Point:
    ...
    def get_neighbours(self, grid: Grid) -> list[Point]:
        """
        Finds the adjacent neighbours for this point and
        returns them as a list.

        If the calculated neighbours lie outside the grid,
        they will be ignored.

        The grid is assumed to be 2D.
        """
        if len(grid.array.shape) != 2:
            raise ValueError("get_neighbours only works for 2D arrays.")

        # Indices outside of these bounds will cause an index error

        max_row = grid.array.shape[0] - 1
        max_col = grid.array.shape[1] - 1
        min_row = 0
        min_col = 0
...
class Grid:
    ...
    def __post_init__(self) -> None:
        self.max_row = self.array.shape[1] - 1
        self.max_col = self.array.shape[0] - 1
        self.min_row = 0
        self.min_col = 0
    ...
    def parse(cls, text: str) -> Grid:
        ...
        """
        array = [list(row) for row in text.strip().splitlines()]

        return Grid(array=np.array(array, dtype=np.int32))

    def get(self, point: Point) -> int:
        """
        Gets the energy level of the octopus at `point`.
        """
        ...
        if not all(
            ...
            raise ValueError(f"Point {point!r} is out of bounds")

        val: int = self.array[point.row][point.col]
        return val

    def set(self, point: Point, value: int) -> None:
        """
        Set the energy level of the ocotopus at `point`
        to `value`.
        """
        # Out of bounds checking
        if not all(
    ...
    def increment(self) -> None:
        """
        Increments the energy level of every octopus in the
        grid.
        """
        # Numpy's universal functions are amazing here
        self.array += 1

    def step(self) -> int:
        """
        Advances the step of the Grid and returns the number
        of flashes.

        A step consists of the following actions:
        1) The energy level of each octopus is incremented by 1
        2) Any octopus with an energy level > 9 "flashes", a flash
            will increment the energy level of it's neighbour octopi,
            if any of them now have > 9 energy, they also flash and so on
        3) After an octopus has "flashed" it's energy level is reset to 0
        """
        ...
        while True:
            to_flash = [
                ...
                if self.array[i][j] > 9 and Point(i, j) not in flashed
            ]
            ...
            for point in to_flash:
                flashed.add(point)
                # Bump the neighbours
                neighbours = point.get_neighbours(self)
                for neighbour in neighbours:
                    self.set(neighbour, self.get(neighbour) + 1)
    ...
    def sync_point(self) -> int:
        """
        Returns the first step number during which
        all octopuses flash.
        """
...
if __name__ == "__main__":
    ...
    INPUT = HERE / "day11.txt"

    with open(INPUT) as f:
        grid = Grid.parse(f.read())

    print(f"Part 1: {sum(grid.step() for _ in range(100))}")
    print()

    # Have to refresh the grid to get the right answer
    with open(INPUT) as f:
        grid = Grid.parse(f.read())

    print(f"Part 2: {grid.sync_point()}")
