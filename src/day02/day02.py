...
class Position:
    ...
    # Note: I've changed the implementation of Position in place for
    # Part 2 so it will no longer work for Part 1
    ...
    def forward(self, amount: int) -> None:
        """
        Increments the horizontal position by `amount` and
    ...
    def up(self, amount: int) -> None:
        """
        Decrements the aim by `amount`.
        """
        self.aim -= amount

    def down(self, amount: int) -> None:
        """
        Increments the aim by `amount`.
        """
        self.aim += amount

    def move(self, move: str) -> None:
        """
        Applies a 'movement' based on the string, a valid
        movement looks like `forward 5` or `up 6`.
        """
        # Split the move on space
        direction, amount = move.split(" ")

        if not direction or not amount:
            raise ValueError(f"malformed movement: {move!r}")

        # Map of the direction to what method it should call
        switch = {
        ...
        if mover := switch.get(direction):
            mover(int(amount))
        else:
            raise ValueError(f"unhandled movement: {direction!r}")

    def apply_movements(self, movements: list[str]) -> None:
        """
        Applies a series of 'movements' from their string
        representations e.g. ["forward 5", "up 2", "down 1"]
        """
...
if __name__ == "__main__":
    ...
    INPUT = HERE / "day02.txt"

    with open(INPUT) as f:
        movements_text = f.read()

    # Mine has 1000 lines
    movements = [movement for movement in movements_text.splitlines()]
    assert len(movements) == 1000

    # Start from 0
    p = Position()

    # Apply the list of movements
    p.apply_movements(movements)

    # Get my answers
    # Part 1 was 1714950, I've changed the Position class in place to do part 2
    # print(f"Part 1: {p.product()}")
    print(f"Part 2: {p.product()}")
